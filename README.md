...
You need [Node.js](https://nodejs.org/) 22 or newer and the [Claude Code CLI](https://claude.com/claude-code) on your `PATH`. Then:

```sh
./start
```

That installs the app's one dependency if needed, starts the server on `http://localhost:4321` and opens it. Two optional extras: `mlx_whisper` and `ffmpeg` for transcribing lecture videos, and `poppler` (`pdftoppm`) so Claude can read PDFs.
...
The code and documentation in this repository — [`app/`](app/), `start`, and [`docs/`](docs/) — are my own work and are MIT-licensed. See [LICENSE](LICENSE).
