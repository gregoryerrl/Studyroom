...
> **Update 2026-08-18 — this table describes the machine the app was BUILT on, not the machines it runs on.** The app now runs on macOS, Linux and Windows; every OS-specific branch lives in `app/server/platform.js`, and the per-OS setup (including the non-Apple transcription engine) is in **[`docs/RUNNING.md`](../RUNNING.md)**, which also states plainly which platforms have actually been executed. The rows below stay as the 2026-08-15 record.
...
1. **Dashboard** — subjects auto-discovered from `$STUDYROOM_DIR/subjects/`. A subject is any directory in there whose name does not start with `.` or `_` (amended 2026-08-16: subjects moved into their own folder, so `docs`/`app` are no longer siblings and no exclusion list is needed). Show per-subject file count and whether `_generated/` has content, and **add / rename / archive subjects from this page**.
...
7. **Subject and file management** (added 2026-08-16, at Gregory's request) — add / rename / archive a subject from the dashboard; upload files (button or drag-drop onto the file pane), write a new markdown note, edit any text-ish file in place (markdown / text / html; PDFs and videos are not editable), rename, and archive, all from the subject page. Removals are archive-moves, never deletes (§5).
...
Plus **`./start`** at the **repo root** (not inside `app/`): a small script that installs deps if `app/node_modules/` is missing, starts the server, and opens `http://localhost:4321`. *(Updated 2026-08-18: the logic moved into `app/start.js` so one implementation serves every OS — `open`, `curl`, `seq`, `sleep` and bash are all macOS/POSIX-only. `./start` and `studyroom.cmd` are one-line shims onto it.)* That's Gregory's day-to-day entry point.
...
- `<root>/subjects/` — **every subject folder lives in here** (amended 2026-08-16; they used to sit at the data root). The root keeps only `profile.md`, `.studyroom/`, `app/`, `docs/` and `start`, so nothing at the root has to be excluded from discovery any more. Created on boot if missing.
- `<root>/subjects/<Subject>/` — materials, owned by Gregory. **Claude never modifies, renames or deletes these** — it may only CREATE, and only under `_generated/` (§6.1, enforced in the system prompt). **The app may, at Gregory's explicit request** (amended 2026-08-16): the UI can upload files, edit text files in place, rename them, and remove them. "Remove" is an **archive-move, never a delete** — the file or folder is renamed into `<root>/.studyroom/archive/`, which is gitignored and restorable by hand, so no app action can actually destroy a material. In-place edits overwrite; git is the undo for anything tracked.
- `<root>/subjects/<Subject>/_generated/` — Claude's outputs, markdown files named `<type>-<topic>-<YYYY-MM-DD>.md` (e.g. `flashcards-eigenvalues-2026-08-15.md`). Claude creates this dir itself via its Write tool.
- `<root>/subjects/<Subject>/_generated/transcripts/<video-basename>.md` — app-produced (not Claude-produced) lecture transcripts, §6.5. Text, small, and **committed to git** — the durable, searchable form of the un-pushable videos.
- `<root>/subjects/<Subject>/_generated/digest-<topic>-<date>/` — a Digest action's study kit: `notes.md`, `visual.html`, `worked-examples.md`, `flashcards.md`, `quiz.md`.
- `<root>/.studyroom/archive/` — where removed things go: `subjects/<name>-<stamp>/{materials,chats}` for a whole subject, `files/<Subject>/<stamp>/<relative path>` for one file.
...
- **Update 2026-08-18:** mlx-whisper is Apple-Silicon-only, so there is now a second engine — **openai-whisper** (`whisper`, any OS, same `large-v3-turbo` weights, underscored flag dialect), selected by whichever is on PATH and overridable with `STUDYROOM_WHISPER` / `STUDYROOM_WHISPER_MODEL`. The SRT and progress-line formats are identical, so everything below still applies verbatim; see `app/server/transcribe.js`'s engine table and [`docs/RUNNING.md`](../RUNNING.md) §3.
...
| `/api/subjects` | POST | body `{name}` — create a subject folder. 400 on an invalid name, 409 if one exists (matched **case-insensitively**: macOS resolves `ai201` to `AI201`'s folder) |
| `/api/subjects/:s` | PATCH | body `{name}` — rename the folder and migrate its chat state (records + `chats/<Subject>/`). 409 on a name clash or while a chat/transcription in it is running |
| `/api/subjects/:s` | DELETE | **archive-move** to `.studyroom/archive/subjects/<name>-<stamp>/{materials,chats}` — never `rm`. Same 409s as PATCH |
...
| `/api/subjects/:s/files/*` | PUT | **the raw body IS the file** — upload and text-save share this route, streamed to disk (no multipart parser, no dependency, a 350 MB video never buffers). 409 if it exists unless `?overwrite=1`. **Must be registered before `express.json()`**, which otherwise consumes the stream for `application/json` and hangs the request forever |
| `/api/subjects/:s/files/*` | PATCH | body `{path}` — rename/move within the subject; re-points any focused chat at the new path |
| `/api/subjects/:s/files/*` | DELETE | **archive-move** to `.studyroom/archive/files/<Subject>/<stamp>/<path>` — never `rm`. 409 if that exact file is being transcribed |
...
Bind to `127.0.0.1:4321` only. **Path safety differs between read and write:** the read route realpaths the *target*, which on a write does not exist yet — so the write routes realpath the *parent*, require it inside the subject's real dir, and rebuild the target from it (a symlinked intermediate directory therefore cannot redirect a write). Any path segment starting with `.` is rejected, which kills `..` traversal and dotfiles in one rule. JSONL persistence timing: the user message when the POST arrives; `tool_use` lines as they stream; at `result`, one assistant line with all text blocks concatenated in arrival order. History must survive a mid-turn crash with at least the user side intact.
